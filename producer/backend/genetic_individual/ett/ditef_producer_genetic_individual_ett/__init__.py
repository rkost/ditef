...
class Individual(ditef_producer_shared.genetic_individual.AbstractIndividual):
    ...
    async def evaluate(self):
        self.evaluation_result = await self.task_api_client.run(
            'ditef_worker_genetic_individual_ett',
            {
                'id': self.id,
                'genome': self.genome,
    ...
    def fitness(self) -> typing.Optional[float]:
        ...
        if self.evaluation_result['compiledNN_result'] > self.configuration['compiledNN_threshold']:
            return 0
        return - self.evaluation_result['loss'] - (self.computational_cost() * self.configuration['computational_cost_factor'])
